...
- `<S-Tab>` : 前のリンクへ移動
- `<Space>` : 文中 / している / されている のリンク一覧ポップアップ（既定で画面いっぱいの2ペイン、左が一覧・右が大きめプレビュー）。区切りごとに `¶`（文中）/`→`（している）/`←`（されている）＋ラベル＋表示名。可視行の先頭に1キーのラベル（`1`〜`0` のあと英字）が付く。
  - `j`/`k` 選択（プレビュー追従）、`g`/`G` 一覧の先頭/末尾
  - `l` またはラベルキー … その行のリンク先へ**潜る**（ポップアップは開いたまま、一覧が飛び先ノートの関係リンクに差し替わり、タイトルのパンくずが伸びる。バッファは開かない）
  - `h` … 1つ戻る（来歴が無ければ閉じる）
  - `⏎` … 選択中のリンク先を実際に開いて**着地**（潜った先で押せばそこから直行）。潜った先にリンクが無いときはそのノート自体へ着地
  - もう一度 `<Space>` … いま潜っているノート自体を開いて着地（起点＝潜っていないときは `⎋` と同じ）
  - `⇥` … 全ノート検索（グローバル）へ切替。打つと絞り込み → `⏎` で選択モード → `l`/`␣` でそのノートを潜る（ポップアップは開いたまま local に戻る）/ `⏎` で開く / `i` で検索に戻る / `⇥`・`h` でローカルへ。パンくずは一覧の1行目に表示
  - `/` … 絞り込み（ラベル＋表示名を部分一致でフィルタ、`⏎` 確定・`⎋` 解除。数字/英字はフィルタに食われず常にジャンプ）
  - `⎋` … 何ホップ潜っても全部取り消して元のノート・カーソル位置へ
  - `^F`/`^B` … プレビュー送り
  - リンクが1つも無ければカーソル巡回にフォールバック。`g:yurii_pkm_space_fullscreen=0` で従来の中央小窓
