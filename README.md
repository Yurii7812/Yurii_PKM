...
- `<S-Tab>` : 前のリンクへ移動
- `<Space>` : parent/child（`している`/`されている`）のリンク一覧ポップアップ。`している（parent）`/`されている（child）` に分けて `ラベル 表示名` を番号付きで表示。`1`〜`9`/`0` で選択（同じ数字をもう一度、または `⏎` で開く）、`j`/`k` で上下、`⎋` か `<Space>` で閉じる。v2 ノートでなければカーソル巡回にフォールバック
