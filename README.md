...
- `<S-Tab>` : 前のリンクへ移動
- `<Space>` : ノートナビゲータ（画面いっぱいの2ペイン。左が一覧、右がプレビュー）。**スコープ**（ローカル＝今のノートのリンク / グローバル＝全ノート検索）と**サブモード**（入力＝打つと絞り込み / 選択＝プレーンキーでナビ）の2軸。キーは両スコープ共通、Ctrl は使わない
  - 一覧の1行目は常に **アンカー**（`◎` ＝ いま潜っているノート）。リンク追加はここに効く
  - ローカルは `── 文中 ──` / `── している ──` / `── されている ──` の3区切り（`¶` `→` `←`）
  - **選択サブモード**
    - **ラベルキー**（`1`〜`0` のあと英字）… **1タップでその行に潜る**。潜ってもノートはアンカー行とプレビューに出続けるし `⌫` で即戻れるので、確認の1打は要らない
    - `j`/`k` … 選択（プレビューが追従）/ `g`/`G` … 一覧の先頭 / 末尾
    - `f`/`b` … プレビュー送り
    - `l` … 選択を潜る（ラベルと同じ。バッファは開かない）
    - `⌫` / `h` … 1つ戻る（グローバルから潜っていれば検索結果へ、クエリ保持）
    - `⏎` … 選択を開いて閉じる。潜った直後はアンカーが選ばれているので、`4` `⏎` で「4番に潜る→そこに着地」
    - `␣` … アンカー自体を開く
    - `c` / `p` … 選択（またはマーク全部）を**アンカーの子 / 親**として追加。関係を数字で選ぶ
    - `y` … 選択（またはマーク全部）を `[タイトル](パス)` でヤンク
    - `m` … マークをトグル（`*` 表示・複数可）/ `M` … 全解除
    - `a` … 選択をアンカーにする
    - `i`（`/`）… 入力サブモードへ / `⇥` … ローカル ⇄ グローバル / `⎋` … 全部取り消して元へ
  - **入力サブモード** … 打ちながら矢印で操作でき、モードを抜けずに完結する
    - 打つ … 絞り込み（グローバルはタイトル＋本文の AND 検索）/ `⌫` … 1字消す
    - `↑`/`↓` … 選択（プレビュー追従）/ `→` … 潜る（1タップ）/ `←` … 戻る
    - ※入力中は英数字がクエリに入るのでラベルは効かない。ラベルで潜りたいときは `⎋` で選択サブモードへ
    - `⏎` … 選択を開いて閉じる（＝選択サブモードと同じ意味）
    - `⇥` … スコープ切替 / `⎋` … 入力を抜けて選択サブモードへ（クエリは残る）
  - グローバルの並びは**最近訪問した順**に固定（無クエリでも絞り込み後でも安定する）
  - 上から **パンくず → 検索バー（右端に件数 / 位置）→ 一覧 → ヘルプ**。打つ場所と見る場所が近い。リンクが1つも無ければカーソル巡回にフォールバック。`g:yurii_pkm_space_fullscreen=0` で従来の中央小窓
- `gs` : 同じナビゲータを**グローバル**（全ノート検索）で開く。ヒットから `→`/`l` でそのまま潜れる。旧ポップアップに戻すなら `g:yurii_search_legacy = 1`
...
- `1`〜`9` : 本文（`<!-- している -->` より前）の N 番目のリンクへジャンプ。本文リンクが無い箇所ではそのままカウント（`5j` 等）として働く
- `<Enter>` : カーソル下リンクを開く
