...
- `n` / `N` : 素のVimの検索リピート（前方/後方）。ノート作成系の2文字コマンド（`zc`/`zp`等）は頭文字を`z`にしてあるため`n`とは衝突しない
...
| `g:yurii_pkm_timeoutlen` | `500` | `&timeoutlen`（ms）をこの値に短縮。`zc`/`ca`等の2文字マッピングと素の`z`/`c`等の単発コマンドが1文字目を共有するため、既定の1000msだと素のキーの反応が遅く感じる。短すぎると`zt`等が2文字目の入力前にタイムアウトして無反応になるので、体感と両立する値として500に調整。`0`で変更しない |
