...
`Up / Down / Back` を分けることで、
「親へ戻る」「子へ進む」「どこから参照されているかを確認する」を明確に使い分けできます。

## Up と BackLink（Back）の仕様

### Up の仕様

- 役割: **親ノートへの導線**。
- `Up` は **複数リンクを保持可能**（複数の親文脈を許可）。
- 主な追加方法:
  - ノート作成コマンド（`nc`, `nq` のモード）で自動挿入
  - `cu` / `:CA` でクリップボードのリンクを Up に追加
  - `ca` / `:CU` でクリップボードのリンクを現在ノートの Down に追加し、リンク先ノートの Up に現在ノートを追加
  - `at` / `:AT` でクリップボード先ノートの Down に現在ノートを追加し、現在ノートの Up にクリップボード先ノートを追加
- 使い方:
  - 現在ノートの文脈上の「上位」へ戻るための導線として運用
  - `bu`（`JumpLastLinkBeforeUp`）で Up 近傍のリンクへ素早く移動
  - `ml`（`:JumpUp`）で、Up リンクが1つだけならリンク先へ直接移動し、複数ある場合は `# Up` 見出しへ移動
  - 親文脈を増やしたい場合は、Up にリンクを追加できる
  - Down からの親子関係が複数ある場合、同期後も Up に複数リンクとして反映
  - PKM ルート配下のサブフォルダをまたぐ Down/Up 関係も、相手ノート基準の相対パス付きリンクとして反映
  - Down からリンクを削除した場合も、同期後に相手ノートの Up から対応リンクが消える
  - Up からリンクを削除した場合も、保存時同期後に相手ノートの Down から対応リンクが消える
  - 未保存の Down 編集がある状態でリンク移動や履歴戻りをした場合も、移動前に保存と同期を行って Up に即反映する
...
### Up と BackLink の違い（重要）

- **Up**: このノートから見た「親方向」の明示リンク（能動的に置く）
...
- `nf` : クイックノート作成（タイトルなし）
- `nn` : プレフィクスなしの新規ノート作成（YAML `filetype: N`）。親ノート側のリンクは `# Down` に作成
...
- `<BS>` : 履歴を戻る
- `bu` : `# Up` 近傍のリンクへ移動
- `mj` / `:JumpUp` : Up リンクが1つだけならリンク先へ直接移動、複数ある場合は一番上の Up リンクへ移動
- `mk` / `:JumpDownTop` : `# Down` セクションの先頭行へ移動
- `ml` / `:JumpDownBottom` : Up リンクが複数ある場合は一番上の Up リンクへ移動し、それ以外は `# Down` セクションの最後尾行へ移動
...
- `bc` / `:BC` : クリップボードのファイルを Down に追加
- `ca` / `:CU` : クリップボードのリンクを現在ノートの Down に追加し、リンク先ノートの Up に現在ノートを追加
- `at` / `:AT` : クリップボード側ノートの Down に現在ノートを追加し、現在ノートの Up にクリップボード側ノートを追加
- `yn` / `:YN` : 現在ファイル名をヤンク
