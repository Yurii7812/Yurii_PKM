...
- `1`〜`9` / 文字+数字（`a1`〜） : 本文 → `Parent`/`Child` の順で通し番号にしたリンクへ移動。**リンクの手前にラベルが仮想テキストで表示される**ので、数えずにどのキーを押せばいいか分かる。1〜9番目は数字1打、10番目以降は文字+数字の2打（`a1`〜`a9`、`b1`…）。該当リンクが無いキーはそのまま素通しされる（`1`〜`9` はカウント `5j` 等、文字はその文字本来の動作）。表示は `g:yurii_pkm_link_hints = 0` で無効化できる（移動自体は無効化されず、番号は数えて押す必要がある）
- `<Enter>` : カーソル下リンクを開く
