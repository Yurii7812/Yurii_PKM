...
- `<S-Tab>` : 前のリンクへ移動
- `<Space>` / `gs` : ノートナビゲータ。**モードは無い。**「ローカル」と「グローバル」の2つのツールがあり、それぞれの中では **1キー＝1つの意味**に固定されている。`⇥` でもう一方へ
  - 見分け方は上のバー。`◎ ノート名`（カーソル無し）＝ 打たないツール / `検索 …▏`（カーソル有り）＝ 打つツール
  - **ローカル**（`<Space>`）… 今のノートのリンク。**打たない。全部コマンド**
    - **ラベルキー**（`1`〜`0` のあと英字）… **1タップでその行に潜る**。潜ってもそのノートはアンカー行 `◎` とプレビューに出続け、`⌫` で即戻れる（非破壊）ので確認の1打は要らない
    - `j`/`k` 選択 ・ `g`/`G` 先頭/末尾 ・ `f`/`b` プレビュー送り
    - `⏎` 選択を開いて閉じる（潜った直後はアンカーが選択済みなので `4` `⏎` で「4番へ潜って着地」）
    - `⌫` / `h` / `←` 戻る ・ `␣` アンカー自体を開く
    - `c`/`p` アンカーの子/親に追加 ・ `y` ヤンク ・ `m` マーク ・ `M` 全解除 ・ `a` アンカー移動
    - `/` この一覧だけの一時絞り込み（`⏎` 確定 / `⎋` 解除）・ `⇥` グローバルへ ・ `⎋` 取消
    - 一覧は `── 文中 ──` / `── している ──` / `── されている ──` の3区切り（`¶` `→` `←`）、1行目は常にアンカー `◎`
  - **グローバル**（`gs`、または `⇥`）… 全ノート検索。**打つ。ラベルは無い**ので数字もそのままクエリに入る
    - 打つ … タイトル＋本文の AND 絞り込み ・ `⌫` 1字消す（空なら戻る）
    - `↑`/`↓` 選択 ・ `→` または `⇥` 選択ノートのローカルへ潜る ・ `⏎` 開いて閉じる
    - `+`/`-` 子/親に追加 ・ `*` マーク ・ `=` ヤンク（記号なので打鍵を邪魔しない）
    - 並びは**最近訪問した順**に固定
  - パンくず（`起点 › … › 現在`）が最上段。`⎋` は何ホップ潜っても全部取り消して元のノート・カーソル位置へ
