...
- 使い方:
  ...
  - `mu`（`:JumpUp`）で、Up リンクが1つだけならリンク先へ直接移動し、複数ある場合は `# Up` 見出しへ移動
...
- `<BS>` : 履歴を戻る
- `bu` : `# Up` 近傍のリンクへ移動
- `mu` / `:JumpUp` : Up リンクが1つだけならリンク先へ直接移動、複数ある場合は `# Up` 見出しへ移動
- `mt` / `:JumpDownTop` : `# Down` セクションの先頭行へ移動
- `mb` / `:JumpDownBottom` : `# Down` セクションの最後尾行へ移動
