...
- 役割: **親ノートへの導線**。
- `Up` は **常に 1 リンクのみ** を保持（単一親文脈）。
- 主な追加方法:
  - ノート作成コマンド（`nc`, `nq` のモード）で自動挿入
  - `cu` / `:CA` でクリップボード先頭リンクへ置き換え、`ca` / `:CU` で Up 直前に追加
- 使い方:
  - 現在ノートの文脈上の「上位」へ戻るための固定導線として運用
  - `bu`（`JumpLastLinkBeforeUp`）で Up 近傍のリンクへ素早く移動
  - 親文脈を変更したい場合は、古い Up を削除して新しい親へ置き換え可能
  - `# Up` 直前行のリンク有無は、Up 親選択の優先条件としては扱わない
  - 置き換えた Up が「実際にその親ノートから参照されている」場合は、同期後もその Up を維持
  - 親ノート側のリンクを A → A' へ移動した場合は、次回同期で子ノートの Up も A' へ追従
