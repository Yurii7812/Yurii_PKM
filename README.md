...
- `<S-Tab>` : 前のリンクへ移動
- `<Space>` : parent/child のリンク一覧ポップアップ（右にプレビュー窓）。各行に `→`（している）/`←`（されている）＋ラベル＋表示名。表示は10行まで、下に `残り N 件`。`j`/`k` で1件ずつ、`h`/`l` で区間（10件）送り、`1`〜`9`/`0` で表示中の行を選択（同じ数字をもう一度、または `⏎` で開く）、`⎋` で閉じる、`^F`/`^B` でプレビューを送る。関係リンクが無ければカーソル巡回にフォールバック
