...
| キー | コマンド | 動作 |
|------|----------|------|
| `\se` | `:SE` | カーソルが開いているSノートをAノートに展開（`expand_s.py` を呼び出す） |
| `mp` | `:RP` | 現在ファイルのプレフィクスを変更し、PKMルート配下の全リンクを一括更新 |

#### `mp` の入力フロー

```
prefix [S] →    → 新プレフィクスを1文字即時入力（a-z、大文字に変換）
```

変更後は新ファイル名のバッファに自動で切り替わり、更新ファイル数が表示される。
...
```
[ファイルstem](ファイル名.md)  タイトル注釈
```

タイトル注釈はスペース2つ以上で区切る。AutoSync により、リンク先のタイトルが変更された場合は自動更新される。
