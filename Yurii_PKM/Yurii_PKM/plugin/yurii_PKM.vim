...
" Minimal bootstrap so the plugin works from a very small vimrc.
...
function! s:setup_conceal() abort
  ...
  " 再実行時の重複定義を防ぐ
  if hlexists('yuriiLinkRegion')
    silent! syntax clear yuriiLinkRegion
  endif
  if hlexists('yuriiLinkText')
    silent! syntax clear yuriiLinkText
  endif
  if hlexists('yuriiConcealOpen')
    silent! syntax clear yuriiConcealOpen
  endif
  if hlexists('yuriiConcealClose')
  ...
  if hlexists('markdownError') | silent! highlight clear markdownError | endif
  if hlexists('htmlError') | silent! highlight clear htmlError | endif
endfunction
